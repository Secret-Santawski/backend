...
class TestFirebaseCRUD(unittest.TestCase):
    ...
    def setUp(self, mock_initialize_app, mock_credentials_certificate, mock_firestore_client):
    ...
    def test_read_document_exists(self):
        ...
        self.assertEqual(response, {"code": 200, "message": "Document read successfully", "data": {"data": "test"}})
    ...
    def test_update_document_success(self):
        ...
        response = self.firebase_crud.update("test_collection", "test_document", {"updated_data": "new_test"})
        self.assertEqual(response, {"code": 200, "message": "Document updated successfully"})

    def test_update_document_not_found(self):
        # Mock Firestore update method to raise NotFound exception
        self.mock_db.collection.return_value.document.return_value.update.side_effect = NotFound('test')

        response = self.firebase_crud.update("test_collection", "test_document", {"updated_data": "new_test"})
        self.assertEqual(response, {"code": 404, "message": "Document to update not found"})
    ...
    def test_delete_document_success(self):
        ...
        self.assertEqual(response, {"code": 200, "message": "Document deleted successfully"})

    def test_delete_document_not_found(self):
        # Mock Firestore delete method to raise NotFound exception
        self.mock_db.collection.return_value.document.return_value.delete.side_effect = NotFound('test')

        response = self.firebase_crud.delete("test_collection", "test_document")
        self.assertEqual(response, {"code": 404, "message": "Document to delete not found"})
